...
with open("token_embeddings.tsv", "w", encoding="utf-8", errors="replace") as f:
    f.write("Token_ID\tToken_Text\tEmbedding_Vector\n")

    for token_id in range(100):
        token_text = tokenizer.convert_ids_to_tokens(token_id)

        embedding = wte[token_id].detach().cpu().numpy()

        embedding_str = ",".join(f"{x:.6f}" for x in embedding)

        f.write(f"{token_id}\t{token_text}\t{embedding_str}\n")
...
for i, token in enumerate(tokens):
    ...
    formatted = [float(f"{x:.4f}") for x in first_10_dims]
    print(f"Token {i}: {repr(token):<15} | First 10 dims: {formatted}")
...
for i in range(seq_len):
    ...
    first_10_dims = pos_embedding[:10].tolist()
    formatted = [round(x, 4) for x in first_10_dims]
    print(f"Position {i}: {repr(tokens[i]):<15} | First 10 dims: {formatted}")
...
for i in range(seq_len):
    ...
    first_10_dims = input_embedding[:10].tolist()
    formatted = [round(x, 4) for x in first_10_dims]
    print(f"Position {i}: {repr(tokens[i]):<15} | First 10 dims: {formatted}")
...
for layer_idx, hidden_state in enumerate(hidden_states):
    ...
    for pos_idx in range(seq_len):
        ...
        first_10_dims = contextual_embedding[:10].tolist()
        formatted = [round(x, 4) for x in first_10_dims]
        print(f"  Position {pos_idx}: {repr(token_name):<15} | First 10 dims: {formatted}")
